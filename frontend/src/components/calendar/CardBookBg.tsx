...
const sizes = {
  sm: { minHeight: 100, minWidth: 80, maxWidth: 120 },
  md: { minHeight: 135, minWidth: 110, maxWidth: 165 },
  lg: { minHeight: 170, minWidth: 140, maxWidth: 200 },
}

interface CardBookBgProps {
...
export default function CardBookBg({ color, size = 'md', className = '', children }: CardBookBgProps) {
  return (
    <div
      className={`relative w-full h-full ${className}`}
      style={{ ...sizes[size], aspectRatio: '201/262' }}
    >
      <svg
        width="201"
        height="262"
      ...
      <div className="absolute inset-0 flex flex-col w-full h-full">{children}</div>
    </div>
  )
}
