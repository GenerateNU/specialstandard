                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {loadingStudents ? (
                        <div className="text-sm text-secondary">Loading students...</div>
                      ) : sessionStudents.length > 0 ? (
                        sessionStudents.map((student: any) => (
                          <div key={student.id} className="flex items-center gap-3 p-2 bg-background rounded-lg border border-default">
                            <Avatar
                              name={getAvatarName(student.first_name, student.last_name, student.id)}
                              variant={getAvatarVariant(student.id)}
                              className="w-8 h-8"
                            />
                            <span className="text-sm text-primary">{student.first_name} {student.last_name}</span>
                            <span className="text-xs text-secondary ml-auto">ID</span>
                          </div>
                        ))
                      ) : (
                        <div className="text-sm text-secondary">No students in this session</div>
                      )}
                    </div>
                  </div>
                </div>
