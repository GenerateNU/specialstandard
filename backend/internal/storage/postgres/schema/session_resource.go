...
func (sr *SessionResourceRepository) GetResourcesBySessionID(ctx context.Context, sessionID uuid.UUID, pagination utils.Pagination) ([]models.Resource, error) {
	...
	query := `
        SELECT r.id, r.theme_id, r.grade_level, r.week, r.type, r.title, r.category, r.content, r.created_at, r.updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.ThemeID, &r.GradeLevel, &r.Week, &r.Type, &r.Title, &r.Category, &r.Content, &r.CreatedAt, &r.UpdatedAt); err != nil {
			return nil, err
		}
